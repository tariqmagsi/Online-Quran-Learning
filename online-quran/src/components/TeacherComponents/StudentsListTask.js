...
class Task extends React.Component {
  ...
  searchStudents = () => {
    ...
    fetch(
      "/searchenrolledcourselists/" +
        ...
        "?isEnrolled=true&&isTeacher=false&&name=" +
        nameInput +
        "&&email=" +
        emailInput +
        "&&contact=" +
  ...
  fetchData = () => {
    fetch(
      "/enrolledcourselists/" +
        ...
        "?isEnrolled=true&&isTeacher=false"
    )
      .then(res => res.json())
      .then(json => {
        if (json.success) {
  ...
  render() {
    ...
    return (
      <div id="enroll">
        <div className="Homepage-heading">Students List</div>
        <div
          className="Homepage-onlineInfo"
          style={{ paddingBottom: "20px", color: "black" }}
        >
          <strong> NOTE: If Students not found then reload the page</strong>
        </div>
        <div
          className="Homepage-onlineInfo"
          style={{ paddingBottom: "20px", color: "green" }}
        >
          <strong> Total Number Of Students = {this.state.count}</strong>
        </div>
